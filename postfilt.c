 ...
 *---------------------------------------------------------------------------*/
  ...
  {                               /* switch off pitch postfilter */
    ...
    if (sub(cmax, en) > 0)      /* nếu gain pitch > 1 */
    {
        g0 = INV_GAMMAP;       // g0 được gán trực tiếp
        gain = GAMMAP_2;       // gain được gán trực tiếp
    }
    else {
        /*-------------------------------------------------------------*
         * Tối ưu hóa phép nhân và phép chia
         *-------------------------------------------------------------*/

        cmax = shr(mult(cmax, GAMMAP), 1);  /* cmax(Q14) = cmax(Q15) * GAMMAP */
        en = shr(en, 1);                      /* en(Q14) = en(Q15) / 2 */

        /* Sử dụng một phép toán cho phép cộng cmax và en */
        Word16 sum = add(cmax, en);          // Tính tổng cmax và en

        if (sum > 0)  /* Kiểm tra xem tổng có lớn hơn 0 không */
        {
            gain = div_s(cmax, sum);          /* gain(Q15) = cor_max/(cor_max+ener)  */
            g0 = sub(32767, gain);            /* g0(Q15) = 1 - gain */
        }
        else
        {
            g0 = 32767;                        /* Thiết lập g0 khi tổng không hợp lệ */
            gain = 0;                          /* Thiết lập gain bằng 0 */
        }
    }
...
)
{
  ...
  if (s == 0) {
    ...
    *---------------------------------------------------*/
    ...
    s = (Word32)(div_s(gain_out,gain_in));   /* Q15 */
