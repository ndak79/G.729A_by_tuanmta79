 ...
 *------------------------------------------------------------------------*/
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
        ...
        if (i_subfr == 0)
        {
            ...
            else  /* Khung bị lỗi hoặc lỗi parity */
            {
                T0 = old_T0;
                T0_frac = 0;
                old_T0 = add(old_T0, 1);
        ...
        else  /* Xử lý cho subframe thứ hai */
        {
            if (bfi == 0)
            {
                Dec_lag3(index, PIT_MIN, PIT_MAX, i_subfr, &T0, &T0_frac);
        ...
        *T2++ = T0;
        ...
        /* Tìm vector adaptive codebook */
        Pred_lt_3(&exc[i_subfr], T0, T0_frac, L_SUBFR);

        /* Giải mã innovative codebook và xử lý khung lỗi nếu có */
        if (bfi != 0)  /* Bad frame */
        {
        ...
        /* Tính toán và áp dụng pitch sharpening */
        j = shl(sharp, 1);  /* From Q14 to Q15 */
        if (sub(T0, L_SUBFR) < 0)
        {
            for (i = T0; i < L_SUBFR; i++)
            {
                code[i] = mac(code[i], code[i - T0], j);
            }
        }

        /* Giải mã pitch và codebook gains */
        index = *parm++;  /* index of energy VQ */
        Dec_gain(index, code, L_SUBFR, bfi, &gain_pitch, &gain_code);

        /* Cập nhật giá trị "sharp" */
        sharp = gain_pitch;
        sharp = (sub(sharp, SHARPMAX) > 0) ? SHARPMAX : sharp;
        sharp = (sub(sharp, SHARPMIN) < 0) ? SHARPMIN : sharp;

        /* Tính toán excitation tổng */
        for (i = 0; i < L_SUBFR; i++)
        {
        ...
        /* Kiểm tra overflow và thực hiện lọc nếu cần */
        Overflow = 0;
        Syn_filt(Az, &exc[i_subfr], &synth[i_subfr], L_SUBFR, mem_syn, 0);
        if (Overflow != 0)
        {
            /* Nếu bị overflow trong quá trình lọc */
        ...
        Az += MP1;  /* pointer to interpolated LPC parameters for next subframe */
    }
