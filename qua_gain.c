...
)
{
 ...
 static Word16 past_qua_en[4] = { -14336, -14336, -14336, -14336 };
   ...
   *-----------------------------------------------------------------*/
   ...
   best_gain[0] = (Word16)( L_acc  >> 16);             /*-- best_gain[0]:Q9 --*/
   ...
   *-----------------------------------------------------------------*/
   ...
   L_acc = L_shr( L_mult( nume,inv_denom ), sft );
   best_gain[1] = (Word16)( L_acc  >> 16);             /*-- best_gain[1]:Q2 --*/
   ...
   if( sub(exp_gcode0,4) >= 0 ){
      ...
      gcode0_org = (Word16)( L_acc  >> 16);              /*-- gcode0_org:Q4 --*/
   }
 ...
 *---------------------------------------------------------------------------*/
   ...
   index2 = cand2;
    ...
    for(i = 0; i < NCAN1; i++) {
        for(j = 0; j < NCAN2; j++) {
            ...
            L_tmp = L_add_Mpy_32_16(L_tmp, coeff[1], coeff_lsf[1], g_pitch);
            L_tmp = L_add_Mpy_32_16(L_tmp, coeff[2], coeff_lsf[2], g2_code);
            L_tmp = L_add_Mpy_32_16(L_tmp, coeff[3], coeff_lsf[3], g_code);
            L_tmp = L_add_Mpy_32_16(L_tmp, coeff[4], coeff_lsf[4], g_pit_cod);
   ...
   *-----------------------------------------------------------------*/
   ...
   tmp = (Word16)( L_shr( L_gbk12,1 ) );                     /* Q12 */
...
)
{
  ...
  *--------------------------------------------------------------------------*/
   ...
   L_acc = L_shl( (Word32)( best_gain[1] ), 7 );    /* L_acc:Q9      */
